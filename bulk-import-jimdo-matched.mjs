...
(async () => {
  const allFiles = fs.readFileSync('/workspace/docs/devoirat/word-files-batch31-jimdo.jsonl', 'utf8')
    .trim().split('\n').map(l => JSON.parse(l));
  
  console.log(`=== PHASE 6: Jimdo Matched Profs ===`);
  console.log(`Total files: ${allFiles.length}`);
  
  const PROGRESS_FILE = '/workspace/docs/devoirat/bulk-jimdo-matched-progress.json';
  const RESULTS_FILE = '/workspace/docs/devoirat/bulk-jimdo-matched-results.jsonl';
  ...
  for (let i = startIdx; i < allFiles.length; i++) {
    ...
    try {
      ...
      const metadata = {
        fileId: 'jimdo-match-' + Date.now() + '-' + Math.floor(Math.random() * 1000),
      ...
      if (i % 50 === 0) {
        console.log(`[${i}/${allFiles.length}] Success: ${successCount}, Fail: ${failCount} | Last: ${result.success ? '✓' : '✗'} ${file.profName} (${elapsed}s)`);
      }
      ...
      if (i % 50 === 0) console.log(`[${i}/${allFiles.length}] ✗ ERROR: ${e.message.slice(0, 60)}`);
