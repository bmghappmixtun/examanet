...
(async () => {
  const allFiles = fs.readFileSync('/workspace/docs/devoirat/word-files-batch31-quad-0.jsonl', 'utf8')
    .trim().split('\n').map(l => JSON.parse(l));
  
  console.log(`=== QUAD 0: ${allFiles.length} files ===`);
  
  const PROGRESS_FILE = '/workspace/docs/devoirat/bulk-jimdo-quad-0-progress.json';
  const RESULTS_FILE = '/workspace/docs/devoirat/bulk-jimdo-quad-0-results.jsonl';
  ...
  for (let i = startIdx; i < allFiles.length; i++) {
    ...
    try {
      ...
      const metadata = {
        fileId: `jimdo-quad0-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
      ...
      if (i % 50 === 0) {
        console.log(`[Q0 ${i}/${allFiles.length}] S:${successCount} F:${failCount} | ${result.success ? '✓' : '✗'} ${file.profName} (${elapsed}s)`);
      }
      ...
      if (i % 50 === 0) console.log(`[Q0 ${i}/${allFiles.length}] ✗ ${e.message.slice(0, 60)}`);
