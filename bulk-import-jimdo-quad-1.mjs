...
(async () => {
  const allFiles = fs.readFileSync('/workspace/docs/devoirat/word-files-batch31-quad-1.jsonl', 'utf8')
    .trim().split('\n').map(l => JSON.parse(l));
  
  console.log(`=== QUAD 1: ${allFiles.length} files ===`);
  
  const PROGRESS_FILE = '/workspace/docs/devoirat/bulk-jimdo-quad-1-progress.json';
  const RESULTS_FILE = '/workspace/docs/devoirat/bulk-jimdo-quad-1-results.jsonl';
  ...
  for (let i = startIdx; i < allFiles.length; i++) {
    ...
    try {
      ...
      const metadata = {
        fileId: `jimdo-quad1-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
      ...
      if (i % 50 === 0) {
        console.log(`[Q1 ${i}/${allFiles.length}] S:${successCount} F:${failCount} | ${result.success ? '✓' : '✗'} ${file.profName} (${elapsed}s)`);
      }
      ...
      if (i % 50 === 0) console.log(`[Q1 ${i}/${allFiles.length}] ✗ ${e.message.slice(0, 60)}`);
