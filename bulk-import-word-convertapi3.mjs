...
const CONVERTAPI_TOKEN = '6U3h59DvBIYZHpH8pN3gjU3R49f0UY05';

const BUDGET = 55;  // 4th free token (250 conversions)
let used = 0;
...
(async () => {
  const allFiles = fs.readFileSync('/workspace/docs/devoirat/word-files-batch30.jsonl', 'utf8')
    .trim().split('\n').map(l => JSON.parse(l));
  
  console.log(`=== PHASE 5: ConvertAPI 87 remaining ===`);
  console.log(`Total Word files: ${allFiles.length}`);
  console.log(`ConvertAPI budget: ${BUDGET}s`);
  
  const PROGRESS_FILE = '/workspace/docs/devoirat/bulk-convertapi3-progress.json';
  const RESULTS_FILE = '/workspace/docs/devoirat/bulk-convertapi3-results.jsonl';
  ...
  for (let i = startIdx; i < allFiles.length; i++) {
    ...
    try {
      ...
      const metadata = {
        fileId: 'conv4-' + Date.now() + '-' + Math.floor(Math.random() * 1000),
      ...
      if (result.success) {
        ...
        console.log(`[${i+1}/${allFiles.length}] ✗ ${file.profName} | Error: ${result.error}`);
      }
      ...
      console.log(`[${i+1}/${allFiles.length}] ✗ ERROR: ${e.message.slice(0, 80)}`);
