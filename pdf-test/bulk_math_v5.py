...
# Vercel proxy (bypasses IP rate limits on direct Blob URL)
...
def chat_with_retry(messages, max_tokens=1500, temperature=0, max_retries=3, base_delay=5):
    """Call OpenAI with retry on 429 rate limit errors."""
...
def sanitize_text(s):
    """Remove control chars that break Neon API (\\x00, \\x01, \\x1a, etc)."""
    if not s:
        return s
    # Strip NULL bytes and other low control chars that crash Neon HTTP API
...
def process_resource(r, log_func):
    ...
    # Use fileUrl (full URL with hash suffix) when available — fileKey may be truncated
    if USE_PROXY and file_url and file_url.startswith(BLOB_BASE):
        # fileUrl is the full Vercel Blob URL — use it via proxy
        url = PROXY_BASE + file_url[len(BLOB_BASE):]
    elif USE_PROXY and file_key:
        # Fall back to fileKey (may be truncated for some files)
    ...
    try:
        ...
        if USE_PROXY and url.startswith(PROXY_BASE):
            req.add_header('X-Internal-Token', INTERNAL_TOKEN)
        with urllib.request.urlopen(req, timeout=60) as resp:
            pdf_bytes = resp.read()
    except Exception as e:
        log_func(f'  Download error for {rid}: {e} | URL: {url[:80]}')
        return False, 0
    ...
    if ok:
        ...
        log_func(f'  ✗ {rid} | FAILED | meta={str(meta)[:200]}')
