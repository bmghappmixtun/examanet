#!/usr/bin/env python3
"""Regenerate ALL weird slugs (without letters or digit-only patterns).
Total ~70 cases."""
...
print(f'Done: {ok} updated, {collisions} collisions handled')

# Verify
r = m.neon_query('''SELECT COUNT(*) FROM "Resource" WHERE slug !~ '[a-z]' AND slug != '' ''')
for row in r.get('response', [{}])[0].get('data', {}).get('rows', []):
    print(f'Remaining weird slugs: {int(row[0])}')
