...
def generate_title(type_, hwn, hwsub, db_subj, cls, sec, year, trim):
    type_label = TYPE_LABELS.get(type_, 'Document')
    if hwsub and 'correction' in hwsub.lower():
...
def main():
    ...
    for i in range(0, len(nids), batch_size):
        ...
        nid_list = ','.join(batch)
        r = m.neon_query(f'''
            SELECT r."numericId", r.id, r.title, r.slug, r.type, r.year, r."homeworkNumber", r."homeworkSubtype",
              r.trimester,
        ...
        for row in r.get('response', [{}])[0].get('data', {}).get('rows', []):
            all_info[str(row[0])] = {
                ...
                'type': row[4], 'year': row[5], 'hwn': row[6], 'hwsub': row[7],
                'trim': row[8], 'db_subj': row[9], 'cls': row[10], 'sec': row[11]
            }
    ...
    new_slugs = [u['new_slug'] for u in updates]
    for u in updates:
        nid = u['nid']
        # Check if any other resource has this slug
        slug = u['new_slug']
