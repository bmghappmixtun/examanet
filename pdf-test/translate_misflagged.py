#!/usr/bin/env python3
"""Translate titles for college resources where language is misflagged.
Strategy:
- language != 'ar'
- subject NOT 'francais' or 'anglais' (those are taught in those languages)
- fullText contains significant Arabic content (>30% AR chars)
- title has French major words (Devoir/Serie/Cours/etc)
"""
...
PROGRESS = '/workspace/edutunisie/pdf-test/translate_misflagged_progress.json'
done = {}
...
def translate_full(title, cls, subj, teacher_id=None):
    ...
    title = re.sub(r'\b(?:Mr|Mme|Mlle)\s+[A-Za-zà-ÿÀ-Ÿ\s\-]+(?=\s*$)', '', title).strip()
    ...
    elif re.search(r'S[ée]rie\s+Corrig[ée]e', title, re.IGNORECASE):
        num = re.search(r'N°?\s*(\d+)', title)
        n = f' عدد {num.group(1)}' if num else ''
        type_t = f'سلسلة تمارين مع الاصلاح{n}'
...
# Get targets: language != 'ar', subject NOT in (francais, anglais), fullText has Arabic
r = m.neon_query('''
SELECT r.id, r."numericId", r.title, c.slug as cls, s.slug as subj, r."teacherId", r.language
FROM "Resource" r
...
WHERE c.slug IN ('7eme', '8eme', '9eme')
  AND r.language != 'ar'
  AND s.slug NOT IN ('francais', 'anglais')
  AND rc."fullText" ~ '[\\u0600-\\u06FF]'
  AND r.title ~ '[A-Za-z]'
  AND r.title ~* 'Devoir|Serie|Cours|Controle|Synthese'
ORDER BY r."numericId"
''')
targets = [{'id': r[0], 'nid': r[1], 'title': str(r[2]), 'cls': r[3], 'subj': r[4], 'teacherId': r[5], 'language': r[6]} 
...
for t in targets[:10]:
    ...
    print(f'  NID {t["nid"]} (lang={t["language"]}, {t["cls"]}/{t["subj"]}): {t["title"][:60]}')
    print(f'    → {new}')
