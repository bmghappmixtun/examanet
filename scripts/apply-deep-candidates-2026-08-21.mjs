...
async function main() {
  const csv = fs.readFileSync('scripts/deep-candidates-2026-08-21.csv', 'utf8');
  const lines = csv.split('\n').filter(Boolean);
  const header = lines[0].split(',');
  
  const targets = lines.slice(1).map(l => {
    const parts = l.split(',');
    return { id: parseInt(parts[0]), fr: parts[1]?.replace(/^"|"$/g, ''), newName: parts[2]?.replace(/^"|"$/g, ''), count: parseInt(parts[4]) };
  });
