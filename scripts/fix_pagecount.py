...
with open('/tmp/files.json') as f:
    files = json.load(f)
# Remove the dotenv output line at start
if isinstance(files, list) and len(files) > 0 and 'id' not in files[0]:
    # Try to find a JSON array
    txt = open('/tmp/files.json').read()
    # Find the first '['
    idx = txt.find('[')
    if idx > 0:
        files = json.loads(txt[idx:])
...
# Save results
with open('/tmp/pagecount_results.json', 'w') as f:
    json.dump(results, f)
