...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    values.push(id);
    const r = await d1Run(`UPDATE Subject SET ${fields.join(', ')} WHERE id = ?`, ...values);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await params;
    // Check usage
    const used = await d1First('SELECT COUNT(*) as c FROM Resource WHERE subjectId = ?', id);
    if (used && Number(used.c) > 0) {
      return NextResponse.json(
        { error: `Impossible : ${used.c} ressource(s) utilisent cette matière` },
        { status: 400 },
      );
    }
    const r = await d1Run('DELETE FROM Subject WHERE id = ?', id);
    if (!r.success) return NextResponse.json({ error: r.error }, { status: 500 });
    return NextResponse.json({ success: true });
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 500 });
