...
function buildNewTitle(r: {
  ...
  const { title, generalSubject, subjectSlug, classSlug, className, sectionSlug, sectionName, schoolType, hasCorrection } = r;
  if (!generalSubject) return null;

  const subjectLabel = subjectSlug === 'economie' ? 'Économie' : (subjectSlug === 'gestion' ? 'Gestion' : subjectSlug);
  ...
  if (typeInfo.kind === 'COURSE') {
    ...
    baseTitle = `Cours - ${subjectLabel}: ${generalSubject} - ${classShort}${sectionPart} (${r.year})`;
  } else if (typeInfo.kind === 'EXERCISE') {
    const sectionPart = sectionShort ? ` - Section ${sectionShort}` : '';
    baseTitle = `Série d'exercices - ${subjectLabel}: ${generalSubject} - ${classShort}${sectionPart} (${r.year})`;
  } else {
    const sectionPart = sectionShort ? ` - Section ${sectionShort}` : '';
    baseTitle = `${typeInfo.label} N°${numero} - ${subjectLabel}: ${generalSubject} - ${classShort}${sectionPart} (${r.year})`;
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const updateSlugs = body.updateSlugs !== false;
