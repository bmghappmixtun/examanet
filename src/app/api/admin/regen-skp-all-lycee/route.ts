...
const SYSTEM_PROMPT = `Tu es un expert dans le système éducatif tunisien (lycée).

À partir d'un titre de devoir ou de cours, tu dois générer 3 SHORT key points (shortKeyPoints) en FRANÇAIS:
- TERMES courts, 2-3 mots maximum chacun
...
const SUBJECTS = ['svt', 'informatique', 'economie', 'anglais'];
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const subjectFilter = body.subject || null; // null = all 4 subjects

    const allFiles: any[] = await prisma.resource.findMany({
      where: {
        class: { level: { slug: 'lycee' } },
        subject: {
          slug: subjectFilter ? { equals: subjectFilter } : { in: SUBJECTS },
        },
        status: 'PUBLISHED',
      },
      include: { metadata: true, class: true, subject: true },
