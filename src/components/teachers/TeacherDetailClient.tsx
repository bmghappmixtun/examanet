'use client';
// @ts-nocheck
// 2026-08-28: Client component for /fr/professeurs/[numericId]/[slug]
// Fetches data from /api/professeurs/[numericId]/[slug]/data
// All numeric fields guarded with ?? 0
...
import {
  ...
  Download, Eye, Briefcase, MessageSquare, CheckCircle2, Heart,
  Share2, ExternalLink, Users, Sparkles, TrendingUp, Bookmark,
} from 'lucide-react';
...
const TYPE_COLORS: Record<string, string> = {
  ...
  EXERCICES: 'from-emerald-400 to-emerald-600',
  EXAMEN: 'from-rose-400 to-rose-600',
  DEVOIR: 'from-amber-400 to-amber-600',
  FICHE: 'from-purple-400 to-purple-600',
  BAC: 'from-red-500 to-rose-700',
  CONCOURS: 'from-indigo-400 to-indigo-600',
  RESUME: 'from-cyan-400 to-cyan-600',
...
interface PageData {
  ...
  teachingSubjects: { slug: string; nameFr: string; nameAr: string | null; color: string | null; icon: string | null }[];
  teachingClasses: { slug: string; nameFr: string; nameAr: string | null }[];
...
export default function TeacherDetailClient({
    ...
    async function fetchData() {
      try {
        const res = await fetch(
          `/api/professeurs/${numericId}/${slug}/data`,
          { cache: 'no-store' }
        );
        if (cancelled) return;
        if (!res.ok) {
        ...
        if (cancelled) return;
        if (json.error) {
          if (json.error === 'Not found') setNotFoundState(true);
        } else {
          setData(json);
        }
        setLoading(false);
  ...
  if (loading) return <TeacherDetailLoading />;
  if (notFoundState) {
    notFound();
    return null;
  }
  if (!data) return <TeacherDetailLoading />;

  return <TeacherDetailView data={data} />;
}
...
function TeacherDetailView({ data }: { data: PageData }) {
  ...
  const personLd = {
    ...
    ...(teacher.governorate ? { address: { '@type': 'PostalAddress', addressRegion: teacher.governorate, addressCountry: 'TN' } } : {}),
  };
  const breadcrumbLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
            ...
            <div className="flex flex-col sm:flex-row items-start gap-6">
              {teacher.avatarUrl ? (
                <img
                  ...
                  alt={fullName}
                  className="w-24 h-24 lg:w-32 lg:h-32 rounded-2xl object-cover border-4 border-white shadow-lg flex-shrink-0"
                />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h1 className="text-2xl lg:text-4xl font-extrabold text-slate-900 truncate">
                    {fullName}
                  </h1>
                ...
                {teacher.firstNameAr && teacher.lastNameAr && (
                  <p className="text-lg text-slate-600 mb-3" dir="rtl" lang="ar">
                    {teacher.firstNameAr} {teacher.lastNameAr}
                  </p>
                )}
                <div className="flex flex-wrap items-center gap-2 mb-4 text-sm">
                  ...
                  {joinDate && (
                    <span className="inline-flex items-center gap-1 bg-white/70 backdrop-blur border border-amber-200 px-2.5 py-1 rounded-lg text-slate-700">
                      ...
                      Membre depuis {new Date(joinDate).getFullYear()}
                    </span>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <ShareButton
                    title={`${fullName} sur Examanet`}
                    url={`${SITE_URL}/professeurs/${teacher.numericId}/${teacher.slug}`}
                  />
                </div>
              </div>
            </div>
            ...
            <aside className="lg:sticky lg:top-24 lg:self-start space-y-4">
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <div className="space-y-3">
                  <StatCard icon={FileText} value={resourceCount} label="Ressources" color="text-primary-600 bg-primary-50" />
                  <StatCard icon={Users} value={teacher.followersCount} label="Abonnés" color="text-rose-600 bg-rose-50" />
                  <StatCard icon={Heart} value={totalFavorites} label="Favoris" color="text-amber-600 bg-amber-50" />
                  <StatCard icon={Download} value={resources.reduce((a, r) => a + (r.downloadsCount ?? 0), 0)} label="Téléchargements" color="text-emerald-600 bg-emerald-50" />
                </div>
              </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {teachingSubjects.map((s) => (
                      <Link
                        key={s.slug}
                        href={{ pathname: '/fr/matieres/[subject]', params: { subject: s.slug } }}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium hover:scale-105 transition"
                        style={{
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {teachingClasses.map((c) => (
                      <span key={c.slug} className="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-100 text-slate-700 rounded-lg text-sm font-medium">
                        📚 {c.nameFr}
                      </span>
                    ))}
                  </div>
              ...
              {teacher.diploma && (
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Diplôme</h2>
                  <p className="text-sm text-slate-700">{teacher.diploma}</p>
                </div>
              )}
            </aside>
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-extrabold flex items-center gap-2">
                    ...
                    Ressources ({resourceCount})
                  </h2>
                  <Link
                    href={`/fr/ressources?teacherId=${teacher.numericId}` as any}
                    className={`text-sm text-primary-600 hover:text-primary-700 font-semibold ${showToutVoir ? '' : 'hidden'}`}
                    aria-hidden={!showToutVoir}
                    tabIndex={showToutVoir ? 0 : -1}
                  >
                    Tout voir →
                  </Link>
                </div>
                <div className={`space-y-3 ${resources.length === 0 ? 'hidden' : ''}`}>
                  {latestResources.map((r) => (
                    <Link
                      key={r.id}
                      href={{ pathname: '/fr/ressources/[id]/[slug]', params: { id: String(r.numericId), slug: r.slug } }}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {r.description && (
                            <p className="text-sm text-slate-500 mt-1 line-clamp-2" dangerouslySetInnerHTML={{ __html: r.description }} />
                          )}
                          <div className="flex items-center gap-4 mt-2 text-xs text-slate-500 flex-wrap">
                            ...
                            <span className="flex items-center gap-1">
                              <Eye className="w-3 h-3" /> {(r.viewsCount ?? 0).toLocaleString('fr-FR')}
                            </span>
                            <span className="flex items-center gap-1">
                              <Download className="w-3 h-3" /> {(r.downloadsCount ?? 0).toLocaleString('fr-FR')}
                            </span>
                            {r.avgRating > 0 && (
